...
struct Mat3 {
  ...
  static Mat3 fromCols(const Vec3& a, const Vec3& b, const Vec3& c) {
    Mat3 R; R.c0=a; R.c1=b; R.c2=c; return R;
  }
...
static Quat quatSlerp(Quat q0, Quat q1, double t) {
  ...
  if (d < 0.0) { q1.w=-q1.w; q1.x=-q1.x; q1.y=-q1.y; q1.z=-q1.z; d = -d; }

  if (d > 0.9995) {
    Quat q{ q0.w + t*(q1.w-q0.w),
            q0.x + t*(q1.x-q0.x),
            q0.y + t*(q1.y-q0.y),
            q0.z + t*(q1.z-q0.z) };
...
static Vec3 quatToRotvec(Quat q_rel) {
  ...
  if (q_rel.w < 0) { q_rel.w=-q_rel.w; q_rel.x=-q_rel.x; q_rel.y=-q_rel.y; q_rel.z=-q_rel.z; }
...
// ---------------------------
struct PngMemWriter { std::vector<uint8_t> buf; };
                                              ...
                                              int gridN = 7)
{
  ...
  auto gridVals = [&](int n)->std::vector<double> {
    std::vector<double> t;
    t.reserve(size_t(n));
    if (n <= 1) { t.push_back(0.0); return t; }
...
static Vec3 rigNominalPosition(const TrajectoryConfig& cfg, double t01)
{
  ...
  double r = cfg.radiusBase * (1.0 + cfg.radiusAmpFrac * std::sin(2.0*PI*cfg.radiusFreq*t01));

  double cth = std::cos(theta);
  Vec3 dir{ cth * std::cos(phi),
            cth * std::sin(phi),
            std::sin(theta) };

  Vec3 base = cfg.center_w + dir * r;

  Vec3 truck{
    cfg.truckAmp_w.x * std::sin(2.0*PI*t01),
    cfg.truckAmp_w.y * std::sin(2.0*PI*t01 + PI/2.0),
    cfg.truckAmp_w.z * std::sin(4.0*PI*t01)
  };
                                          ...
                                          std::vector<PoseGT>* outGtAtImu = nullptr)
{
  ...
  std::vector<Quat> q(N);
  for (size_t k=0; k<N; ++k) { pos[k] = gt[k].p_w; q[k] = gt[k].q_wi; }
...
int main(int argc, char** argv)
{
  try {
    ...
    const int FPS = 30;
    const int NUM_FRAMES = 240; // 8 seconds @ 30fps
    ...
    const int    AUTOFIT_MAX_ITERS = 18;
    const double AUTOFIT_DIST_GROW = 1.03;
    const double AUTOFIT_OFF_SHRINK= 0.97;
    const double RELAX_DIST = 0.995;
    const double RELAX_OFF  = 0.010;
    ...
    // -----------------------
    Dist5 dist;
    dist.k1 = -0.22; dist.k2 = 0.08;
    dist.p1 =  0.0005; dist.p2 = -0.0003;
    dist.k3 = -0.015;
    ...
    pXY.width_m = wXY_m; pXY.height_m = hXY_m;
    ...
    pYZ.width_m = wYZ_m; pYZ.height_m = hYZ_m;
    ...
    pXZ.width_m = wXZ_m; pXZ.height_m = hXZ_m;
    ...
    traj.truckAmp_w = Vec3{0.22*xmax, 0.22*ymax, 0.10*zmax};

    traj.phi0 = deg2rad(25); traj.phi1 = deg2rad(70);
    traj.th0  = deg2rad(18); traj.th1  = deg2rad(55);
    ...
    // Auto-fit state (continuous)
    double distScale_state = 1.0;
    double offScale_state  = 1.0;
    ...
    for (int f=0; f<NUM_FRAMES; ++f) {
      ...
      Vec3 rigNom_w = rigNominalPosition(traj, t01);

      // Relax toward nominal
      distScale_state = std::max(1.0, distScale_state * RELAX_DIST);
      offScale_state  = std::min(1.0, offScale_state + RELAX_OFF);

      double yawDes, pitchDes, rollDes;
      desiredOffsets(traj, t01, yawDes, pitchDes, rollDes);

      bool ok = false;
      double distScale = distScale_state;
      double offScale  = offScale_state;

      Vec3 rigPos_w;
      Mat3 R_wr_final;
      Mat3 R_rw_final;

      // Auto-fit loop: ONLY change pose (translation+attitude), NOT intrinsics or image scaling
      for (int it=0; it<18; ++it) {
        Vec3 v = rigNom_w - traj.center_w;
        double d = norm(v);
        Vec3 dir = (d > 1e-12) ? (v * (1.0/d)) : Vec3{1,1,1};
        rigPos_w = traj.center_w + dir * (d * distScale);

        rigPos_w.x = std::max(rigPos_w.x, 1e-6);
        rigPos_w.y = std::max(rigPos_w.y, 1e-6);
        rigPos_w.z = std::max(rigPos_w.z, 1e-6);

        Mat3 R_wr_base = lookAt_Rwc(rigPos_w, traj.center_w, worldUp);

        Mat3 R_off = mul(rotZ_cam(offScale * rollDes),
                   mul(rotY_cam(offScale * yawDes),
                       rotX_cam(offScale * pitchDes)));

        R_wr_final = mul(R_off, R_wr_base);
        R_rw_final = transpose(R_wr_final);
        ...
        ok = true;
        for (int i=0; i<4; ++i) {
          if (!allPointsInView(cams[i], checkPts, MARGIN_PX)) { ok=false; break; }
        }
        if (ok) break;

        distScale *= AUTOFIT_DIST_GROW;
        offScale  *= AUTOFIT_OFF_SHRINK;
        distScale = std::min(distScale, 10.0);
        offScale  = std::max(offScale, 0.0);
      }

      distScale_state = distScale;
      offScale_state  = offScale;

      autoCsv << f << "," << time_s << "," << distScale_state << "," << offScale_state << "\n";
      ...
      std::cout << "Frame " << f << "/" << (NUM_FRAMES-1)
                << " distScale=" << distScale_state
                << " offScale="  << offScale_state
                << (ok ? "" : " [WARN fit not perfect]")
                << "\n";
...
#else
              << "ROS bag      : (not built) compile with -DWITH_ROS1 under ROS1\n"
#endif
