...
def run_real_experiment(
    ...
    for name, new_order in order_dict.items():
        ...
        rdg = data_dict.get("RDG", np.nan)
        max_res = data_dict.get("max res", np.nan)
        if np.isfinite(rdg) and np.isfinite(max_res) and abs(rdg) > 1e-1 and abs(max_res) < 1e-5:
...
def create_rmse_table(df, fname_root, add_n_landmarks=False):
    ...
    for i, row in df.iterrows():
        ...
        df.loc[i, "local solution cert"] = row.get(
            f"local solution {local_min_idx} cert", False
        )
