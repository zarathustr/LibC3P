...
class Learner(object):
    ...
    def is_tight(self, verbose=False, data_dict={}, tightness=None):
        ...
        if info["cost"] is None:
            ...
            # Ensure downstream code can rely on these keys even when the SDP solve fails.
            data_dict["d"] = np.nan
            data_dict["RDG"] = np.nan
            data_dict["SVR"] = np.nan
            return False
    ...
    def generate_minimal_subset(
        ...
        if reorder:
            ...
            if lamdas is None:
                ...
                if X is None:
                    print("Sanity checks skipped: SDP solver returned X=None.")
                    return None
                try:
                    xhat_from_X, _ = rank_project(X, p=1)
                except Exception as e:
                    print(f"Sanity checks skipped: rank_project failed: {e}")
                    return None
                xhat = self.solver_vars["xhat"]
                print("max xhat error:", np.min(xhat - xhat_from_X))
                H = info.get("H") if isinstance(info, dict) else None
                if H is None:
                    print("Sanity checks: H is None (solver failed or returned no dual matrix).")
                    return None
                try:
                    print("max Hx", np.max(np.abs(H @ xhat)))
                    print("max Hx_from_X", np.max(np.abs(H @ xhat_from_X)))
                    eigs = np.linalg.eigvalsh(H.toarray())
                    print("min eig of H", np.min(eigs))
                except Exception as e:
                    print(f"Sanity checks skipped: failed to evaluate H diagnostics: {e}")
                return None
    ...
    def find_global_solution(self, data_dict={}):
        ...
        if self.lifter.TIGHTNESS == "rank":
            X = self.solver_vars.get("X") if isinstance(self.solver_vars, dict) else None
            if X is None:
                print("Warning: cannot recover global rank-1 solution because X is None.")
                x = None
            else:
            ...
            try:
                ...
                if eps is not None:
                    cert = abs(eps) <= GLOBAL_THRESH
                    print(f"global solution eps: {eps:.2e}, cert: {cert}")
            except Exception as e:
                print(f"Warning: failed to certify global solution: {e}")
            data_dict["global solution cert"] = cert

            if eps is not None and cert:
                x = xhat
        ...
        keys = [
            ...
            if key.startswith("local solution") and ("cert" not in key)
        ]
        for key in keys:
            ...
            try:
                ...
                eps = info.get("eps", None)
                if eps is not None:
                    print(f"local solution eps: {eps:.2e}")
                    cert = abs(eps) <= GLOBAL_THRESH
            except Exception as e:
                print(f"Warning: failed to certify {key}: {e}")
    ...
    def get_sufficient_templates(self, new_order, new_lifter):
        ...
        if new_order is None:
            # When we fail to compute a minimal subset (e.g., SDP solver failure without a MOSEK license),
            # fall back to "use all constraints" so downstream real-experiment code can still run.
            all_constraints = self.templates_known + self.constraints
            new_order = list(range(len(all_constraints) + 1))  # +1 for A0 at index 0
