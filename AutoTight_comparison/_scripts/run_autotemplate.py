...
def generate_results(lifters_list, seed=0, results_dir=RESULTS_DIR, debug=debug):
    ...
    for Lifter, dict in lifters_list:
        ...
        try:
            ...
            print(f"Warning: failed to compute minimal subset (sorted): {e}")
        t_suff = time.time() - t1
        ...
        if not debug:
            try:
                ...
                print(f"Warning: failed to compute minimal subset (original): {e}")
    ...
    with open(fname, "w") as f:
        for out in (lambda x: print(x, end=""), f.write):
            ...
            for lifter, df_sub in df.groupby("lifter", sort=False):
                ...
                out(_safe_int(df_sub["n constraints"].values[-1]) + " & ")
                out(_safe_int(df_sub["n required"].values[-1]) + " & ")
                for t in times.keys():
