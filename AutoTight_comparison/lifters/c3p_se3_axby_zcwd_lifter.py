...
class C3PSE3AXBYZCWDLifter(StateLifter):
    ...
    def get_x(self, theta=None, parameters=None, var_subset=None) -> np.ndarray:
        ...
        if theta is None:
            theta = self.theta
        # parameters are accepted for compatibility with StateLifter.generate_Y
        # and the AutoTight learner API. This problem currently does not use
    ...
    def test_and_add(self, A_list, Ai, output_poly: bool):
        """Append a known-constraint matrix after a quick self-consistency check.

        This mirrors the helper used by RobustPoseLifter in the original codebase.
        It verifies that the constraint is satisfied by the internally stored ground-truth
        sample (self.theta) in the noise-free sense.

        Parameters
        ----------
        A_list : list
            List of constraint matrices to append into.
        Ai : PolyMatrix
            Polynomial matrix representing the quadratic constraint x^T A x = 0.
        output_poly : bool
            If True, append PolyMatrix objects. If False, append sparse numeric matrices.
        """
        # Use ground-truth theta to sanity check the constraint construction.
        x = self.get_x(self.theta, var_subset=self.var_dict)
        Ai_sparse = Ai.get_matrix(self.var_dict)
        err = float(x.T @ (Ai_sparse @ x))
        if abs(err) > 1e-8:
            raise ValueError(
                f"Known constraint check failed: x^T A x = {err} (should be ~0)."
            )
