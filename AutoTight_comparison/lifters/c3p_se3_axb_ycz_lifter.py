...
class C3PSE3AXB_YCZLifter(StateLifter):
    """
    SE(3) C3P of type AXB = YCZ via unit dual quaternions.

    Unknowns:
        X, Y, Z as unit dual quaternions dqX, dqY, dqZ in R^8.

    Lift:
        uZY = kron(dqZ, dqY) in R^64

    Dual-quaternion bilinear map:
        dq1*dq2 = T_dq @ kron(dq2, dq1)

    Right side:
        Y C Z = Y (C Z)
              = T_dq @ kron(dqC*dqZ, dqY)
              = T_dq @ (kron(L(dqC), I) kron(dqZ, dqY))
              = S uZY

    Left side:
        A X B = R(dqB) L(dqA) dqX = M dqX
    ...
    def var_dict(self):
        if self.var_dict_ is None:
            self.var_dict_ = {"h": 1, "x": 8, "y": 8, "z": 8, "uZY": 64}
    ...
    def sample_theta(self):
        qX, qY, qZ = rand_quat(), rand_quat(), rand_quat()
        tX = np.random.normal(scale=1.0, size=3)
        tY = np.random.normal(scale=1.0, size=3)
        tZ = np.random.normal(scale=1.0, size=3)
        return np.hstack([qX, tX, qY, tY, qZ, tZ])

    def _split_theta(self, theta: np.ndarray):
        theta = np.asarray(theta, dtype=float).reshape(-1)
        assert theta.size == 21
        qX = quat_normalize(theta[0:4])
        tX = theta[4:7]
        qY = quat_normalize(theta[7:11])
        tY = theta[11:14]
        qZ = quat_normalize(theta[14:18])
        tZ = theta[18:21]
        return qX, tX, qY, tY, qZ, tZ

    def _dq_from_theta(self, theta: np.ndarray):
        qX, tX, qY, tY, qZ, tZ = self._split_theta(theta)
        dqX = dq_from_qt(qX, tX)
        dqY = dq_from_qt(qY, tY)
        dqZ = dq_from_qt(qZ, tZ)
        return dqX, dqY, dqZ

    def generate_random_setup(self):
        """
        Sample A_i,B_i randomly and set
            C_i = Y^{-1} A_i X B_i Z^{-1}
        so that AXB=YCZ holds exactly.
        """
        qX, tX, qY, tY, qZ, tZ = self._split_theta(self.theta)
        TX = se3_from_qt(qX, tX)
        TY = se3_from_qt(qY, tY)
        TZ = se3_from_qt(qZ, tZ)
        ...
        for _ in range(self.n_measurements):
            ...
            TB = rand_se3(trans_sigma=1.0)
            left = se3_mul(se3_mul(TA, TX), TB)
            TC = se3_mul(se3_mul(se3_inv(TY), left), se3_inv(TZ))
            A0.append(TA)
            B0.append(TB)
            C0.append(TC)
    ...
    def _coeff_matrices(self, dqA: np.ndarray, dqB: np.ndarray, dqC: np.ndarray):
        M = dq_R(dqB) @ dq_L(dqA)
        K = np.kron(dq_L(dqC), np.eye(8))
        S = self.T_dq @ K
        return M, S

    def get_Q(self, noise: float = None, output_poly: bool = False, use_cliques: list = []):
        if (self._A0 is None) or (self._B0 is None) or (self._C0 is None):
        ...
        for i in range(self.n_measurements):
            ...
            TC = apply_noise_se3(self._C0[i], rot_sigma, trans_sigma)
            A[i] = se3_to_dq(TA)
            B[i] = se3_to_dq(TB)
            C[i] = se3_to_dq(TC)
    ...
    def get_Q_from_y(self, y, output_poly: bool = False, use_cliques: list = []):
        ...
        Q_uu = np.zeros((64, 64), dtype=float)
        Q_xu = np.zeros((8, 64), dtype=float)

        for i in js:
            M, S = self._coeff_matrices(A[i], B[i], C[i])
            Q_xx += M.T @ M
            Q_uu += S.T @ S
            Q_xu += -M.T @ S
    ...
    def get_x(self, theta=None, parameters=None, var_subset=None) -> np.ndarray:
        ...
        dqX, dqY, dqZ = self._dq_from_theta(theta)
        uZY = np.kron(dqZ, dqY)
        blocks = {"h": np.array([1.0]), "x": dqX, "y": dqY, "z": dqZ, "uZY": uZY}
    ...
    def get_error(self, theta_hat):
        ...
        qX_gt, tX_gt, qY_gt, tY_gt, qZ_gt, tZ_gt = self._split_theta(self.theta)
        qX, tX, qY, tY, qZ, tZ = self._split_theta(th_hat)

        rot = np.array(
            [rot_err_deg(qX, qX_gt), rot_err_deg(qY, qY_gt), rot_err_deg(qZ, qZ_gt)]
        )
        tra = np.array([trans_err(tX, tX_gt), trans_err(tY, tY_gt), trans_err(tZ, tZ_gt)])
        return {
    ...
    def residuals(self, theta, y) -> np.ndarray:
        ...
        res = []
        for i in range(self.n_measurements):
            left = dq_mul(dq_mul(A[i], dqX), B[i])
            right = dq_mul(dq_mul(dqY, C[i]), dqZ)
    ...
    def local_solver(self, t0, y, verbose=False):
        ...
        def fun(th):
            ...
            th[14:18] = quat_normalize(th[14:18])
    ...
    def get_A_known(self, var_dict=None, output_poly=False):
        ...
        if ("uZY" in var_dict) and ("y" in var_dict) and ("z" in var_dict):
            for a in range(8):
                for b in range(8):
                    k = a * 8 + b
                    Ai = PolyMatrix(symmetric=True)

                    e = np.zeros((1, 64), dtype=float)
                    e[0, k] = 1.0
                    Ai["h", "uZY"] = 0.5 * e
    ...
    def __repr__(self):
        return f"c3p_se3_axb_ycz_N{self.n_measurements}"
